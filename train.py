...
class Logger:
    ...
    def _print_training_status(self):
        metrics_data = [np.float64(self.running_loss[k]/SUM_FREQ) for k in sorted(self.running_loss.keys())]
...
def train(args):
    ...
    model = nn.DataParallel(LLAFlow(args), device_ids=args.gpus)
    ...
    if args.steps > 0:
        ...
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, args.lr, args.num_steps + 100,
                                                  pct_start=0.05, cycle_momentum=False, anneal_strategy='linear',last_epoch=args.steps-1)
        print('resume training from',args.steps)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--steps', type=int, default=-1, help='resume training steps')
    parser.add_argument('--save_every_epoch', action='store_true', help='save ckpt every epoch')
