...
def PatchExtra(x, size):
    ...
    return F.unfold(x, [size, size], stride=1, padding=0).reshape(B, C, size * size, H, W)


def ImgShift(x, size):
    B, C, H, W = x.shape
    x = F.pad(x, (size // 2, size // 2, size // 2, size // 2), 'replicate')
    return F.unfold(x, [H, W], stride=1, padding=0).reshape(B, C, H, W, size ** 2).permute(4, 0, 1, 2, 3)
